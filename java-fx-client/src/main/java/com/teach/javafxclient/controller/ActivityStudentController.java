...
import com.teach.javafxclient.controller.admin.AddActivityController;
...
public class ActivityStudentController {
    ...
    public void initialize() throws InvocationTargetException, IllegalAccessException {
        ...
        req.put("userId","");
        res = httpRequestUtil.requestArrayList("/api/activity/getActivityListByUserId",req); //从后台获取所有学生信息列表集合
        if(res != null && res.getCode()== 0) {
            activityList = res.getData();
        }
        res =httpRequestUtil.request("/api/activity/getBee",req);
        if(res != null && res.getCode()== 0) {
            Bee=(Map) res.getData();
...
/*        checkColumn.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<StudentTableEntity, Boolean>, ObservableValue<Boolean>>() {
    ...
    protected void onQueryButtonClick() {
        ...
        if (!filterCriteria.isEmpty()){
            ...
            res = httpRequestUtil.requestArrayList("/api/activity/getActivityListByFilter/" ,req);

            //因为有筛选条件，修改一下筛选按钮
            hasFilter();
        }else {
            req.put("userId","");
            res = httpRequestUtil.requestArrayList("/api/activity/getActivityListByUserId",req);
            resetFilter();
        }
        if(res != null && res.getCode()== 0) {
            activityList = res.getData();
    ...
    protected void onAddButtonClick() {
        FXMLLoader fxmlLoader = new FXMLLoader(MainApplication.class.getResource("add-student-activity.fxml"));
        try {
            Parent root = fxmlLoader.load();
            AddStudentActivityController controller = fxmlLoader.getController(); // 获取控制器对象
